...
function Canvas({projectDetail, screenConfig, loading, takeScreenshot}: Props) {
    ...
    useEffect(() => {
        ...
        const saveOnly = takeScreenshot === true;

        const waitForIframes = async () => {
            const iframes = iframeRefs.current.filter(Boolean);
            if (!iframes.length) {
                await new Promise(r => setTimeout(r, 2000));
            }
            await Promise.all(
                iframeRefs.current.filter(Boolean).map(
                    (iframe) =>
    ...
    const onTakeScreenshot = async (saveOnly = false) => {
        try {
            ...
            const headerH = 40;
            const outW = Math.max(iframes.length * (SCREEN_WIDTH + GAP), SCREEN_WIDTH);
            const outH = SCREEN_HEIGHT;
            ...
            // 3) save as JPEG for smaller payload
                ...
                {screenConfig?.map((screen, index) => (
                    screen?.code ? (
                        <ScreenFrame
                            ...
                            screen={screen}
                            iframeRef={(ifrm: HTMLIFrameElement | null) => {
                                if (ifrm) {
